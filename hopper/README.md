...
- **TTL fix is compatible with iPhone tethering**. The `ip ttl set 65` rule targets `oifname "eth1"` (per `configs/nftables/60-mobile-ttl.nft`). The iPhone does NOT rewrite TTL on hotspot egress — it forwards what it receives — so a packet leaving Hopper at TTL 65 reaches the iPhone, the iPhone decrements it to 64 on its own egress to the carrier, and the operator sees the phone-native TTL 64. Same arithmetic as the Huawei path, same correct result. Verified on hardware. So the rule works for both uplinks and needs no per-device gating.
