...
- **WireGuard handshake-only-no-data on some operator SIMs**. Observed empirically: certain RU operator SIMs (one specific MegaFon SIM in the Huawei) pass WG/AmneziaWG initial handshake packets (short UDP, hit DPI's allow-edge cases) but **drop the encrypted bulk-data payload that follows**, in both directions. Symptom: `awg show awg0` reports a fresh handshake and `transfer: ~92 B received` that never grows past the handshake reply size. Looks identical to a misconfigured VPN server but the server is fine — it's the carrier's first-mile filter. Move to a different uplink (different operator's SIM or iPhone tethering) to confirm; if the same tunnel works through one but not the other, the SIM-side carrier is the cause.

### iPhone tethering (USB)

Alternative WAN-of-last-resort: any modern iPhone with an active SIM can substitute for the Huawei E3372. Useful when (a) you don't have the E3372 with you, (b) the E3372's SIM is on a carrier that drops VPN data (see above), or (c) you want a quick "second WAN" without buying another modem.

- **Stack on the router**: `kmod-usb-net-ipheth` (kernel-side USB Ethernet driver for Apple's proprietary tethering protocol) + `usbmuxd` (userspace pairing daemon for the `Trust This Computer?` handshake). Both are installed by `post-upgrade.sh` alongside the Huawei stack — having them on disk is harmless when no iPhone is plugged in.
- **iPhone-side**: `Settings → Personal Hotspot → Allow Others to Join = ON`. On first USB plug-in the iPhone shows a `Trust This Computer?` prompt on its lockscreen — must be unlocked + tapped + passcode entered. Until trust is granted, `dmesg` cycles through `Apple iPhone USB Ethernet device attached` → `now disconnected` every few seconds, and the `eth1` interface stays `NO-CARRIER`.
- **Network**: the iPhone hands out DHCP from `172.20.10.0/28` (Apple's standard hotspot range), with itself as `172.20.10.1`. Hopper's existing `wwan` UCI interface (bound to `device 'eth1'`) picks it up automatically — no config change needed, because both Huawei HiLink and iPhone ipheth enumerate as `eth1` when they're the only USB-Ethernet on the bus.
- **Caveat — interface name collision**: when both an iPhone and a Huawei modem are plugged in simultaneously (two USB ports), one ends up `eth1` and the other `eth2`, with the order depending on enumeration timing. Not deterministic. If you want a stable two-modem topology, see the triple-WAN notes elsewhere in this README.
- **TTL fix does NOT apply to iPhone tethering**. The iPhone's Personal Hotspot egress already presents the phone's native TTL, so the `ip ttl set 65` rule that targets `oifname "eth1"` (per `configs/nftables/60-mobile-ttl.nft`) actually does the wrong thing for iPhone-tethered traffic — it bumps iPhone-originated packets one TTL higher than the phone would natively, which is the opposite of what carriers' tethering-detection expects. In practice this rarely triggers a block (carriers run the heuristic looser for iOS hotspot than for raw Linux clients), but be aware that if a tethering-throttle hits, the rule is the suspect.
...
- **SSID with `@` or `.` works fine** (confirmed end-to-end 2026-05-14). The original Pi README warned against these characters because hostapd with `utf8_ssid=1` (the OpenWrt default) emits `ssid2="..."` instead of `ssid=`, and the Pi-era observation was that macOS / Nintendo Switch silently fail to associate. Re-tested on this Hopper with SSID `f@lex.la-liberated`: hostapd produces the SAME `ssid2="quoted-string"` form (because `@` and `.` are printable ASCII, hostapd quotes rather than hex-encodes), and **Nintendo Switch and macOS associate cleanly and reach the internet**. The Pi-era failure was either BCM43455-specific or a misdiagnosis. The hex form `ssid2=HEX...` (which would actually trip strict clients) only appears for non-printable / non-ASCII codepoints — not for `@` / `.`. So feel free to use domain-style SSIDs.
